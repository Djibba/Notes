...
Une dernière chose importante qu 'il vous faut mémoriser : toute variable non déclarée 
    (donc utilisée sans jamais écrire le mot - clé
        var) deviendra immédiatement une propriété de l 'objet window
...
un des plus utilisés.Et pour cause, il représente la page Web
...
//getElementById()
Cette méthode permet d 'accéder à un élément en connaissant son ID qui est simplement l'
attribut id de
l 'élément. Cela fonctionne de cette manière :

//getElementsByTagName()
var divs = document.getElementsByTagName('div');
Faites très attention dans le nom de cette méthode: il y a un« s» à Elements.C 'est une source fréquente d'
erreurs.
...
name que vous spécifiez.L 'attribut name n'
est utilisé qu 'au sein des formulaires,

Voyons maintenant les particularités de ces deux méthodes.La première, querySelector(), renvoie le premier élément trouvé
correspondant au sélecteur CSS, tandis que querySelectorAll() va renvoyer tous les éléments(sous forme de tableau) correspondant au sélecteur CSS fourni
...
on peut aussi utilisé "textContent"
pour n afficher que le texte situer entre deux balise et non les element html
...
d 'éditer un attribut Le premier paramètre est le nom de l'
attribut, et le deuxième, dans le cas des et Attribute() uniquement,
    ...
    <
    a id = "myLink"
href = "http://www.un_lien_quelconque.com" > Un lien modifié dynamiquement < /a>
...
//NB---------------------------------
Toujours dans le même cas, le nom
for est réservé lui aussi en JavaScript(pour les boucles).Vous ne pouvez donc pas modifier l 'attribut HTML for 
...
présentes, innerHTML les retournera sous forme de texte:
    ...
    Pour éditer ou ajouter du contenu HTML, il suffit de faire l 'inverse, c'
est - à - dire de définir un nouveau contenu:
...
considérablement l 'exécution du code si l'
on opère de cette manière, il vaut donc mieux concaténer
son texte dans une variable pour ensuite ajouter le tout via innerHTML.

Le fonctionnement d 'innerText est le même qu'
innerHTML excepté le fait que seul
...
à l 'exception des versions d'
Internet Explorer antérieures à la 9. Le fonctionnement est évidemment le même.
    //EXEMPLE ILLUSTRATIF
...
body >
    ...
    La propriété parentNode permet d 'accéder à l'
élément parent d 'un élément. Regardez ce code :
    //exemple
    -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    <
    blockquote >
    <
    p id = "myP" > Ceci est un paragraphe! < /p> </blockquote >
    var paragraph = document.getElementById('myP');
var blockquote = paragraph.parentNode;
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --

//nodeType et nodeName
nodeType et nodeName servent respectivement à vérifier le type d 'un nœud et le nom d'
un nœud.
nodeType
...
nodeName, quant à lui, retourne simplement le nom de l 'élément, en majuscule. Il est toutefois conseillé 
...
lastElementChild.Ainsi, dans l 'exemple précédent, la propriété firstElementChild renverrait l'
élément < a > .
...
Il faut soit utiliser la propriété nodeValue soit la propriété data.Si on recode le script précédent, nous obtenons ceci:
    ...
    //childNodes
    La propriété childNodes retourne un tableau contenant la liste des enfants d 'un élément. L'
exemple suivant illustre le fonctionnement de cette propriété, de manière à récupérer le contenu des éléments enfants:
    //---------------------------------------------------
    <
    body >
    <
...
body >
    ...
    div >
    <
    p id = "myP" > Un peu de texte, < a > un lien < /a> et <strong>une portion en emphase</strong > < /p> </div >
...
div >
    <
    p > Paragraphe 1 < /p> <p> Paragraphe 2 </p > < p > Paragraphe 3 < /p> </div >
...
d 'ignorer les nœuds textuels. Ils s'
utilisent exactement de la même manière que les attributs
...
Avec le DOM, l 'ajout d'
un élément HTML se fait en trois temps:

    1 - On crée l 'élément ;

2 - On lui affecte des attributs;

3 - On l 'insère dans le document, et ce n'
est qu 'à ce moment-là qu'
il sera« ajouté».

//--------Création de l'élément
La création d 'un élément se fait avec la méthodecreateElement(), un sous-objet de l'
objet racine,
...
ce qui signifie qu 'il nous faut connaître l'
élément auquel on va ajouter l 'élément créé. Considérons 
...
On va ajouter notre élément < a > dans l 'élément<p>portant l'
IDmyP.Pour ce faire, il suffit de récupérer
...
L 'insertion se fait ici aussi avecappendChild(), sur l'
élémentnewLink.Afin d 'y voir plus clair, 
résumons le code:
    //-----------------------------------------------------------------
    <
    body >
...
newLink.appendChild(newLinkText); < /script> </body >
//----------------------------------------------------------------------//
//RESUMER
var newLink = document.createElement('a');
var newLinkText = document.createTextNode("Le Site du Zéro");
...
Ce qu 'il faut retenir de tout ça, c'
est que les objets du DOM sont toujours accessibles par référence, et c 'est la raison pour laquelle ce code ne fonctionne pas :
...
Dans l 'exemple suivant, le contenu textuel (pour rappel, il s'
agit du premier enfant de < a > ) du lien va être remplacé par un autre.
La méthodereplaceChild() est exécutée sur l 'élément<a>, c'
est - à - dire le nœud parent du nœud à remplacer.
    //--------------------------------------------------------------------------------------------------------
    <
    body >
    <
...
link.replaceChild(newLabel, link.firstChild); < /script> </body >
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //

//Supprimer un élément
Pour insérer un élément, on utilise appendChild(), et pour en supprimer un, on utilise removeChild().Cette méthode prend en paramètre
...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- //
Il n 'y a pas besoin de récupérermyP(l'
élément parent) avecgetElementById(), autant le faire directement avecparentNode.
À noter que la méthoderemoveChild() retourne l 'élément supprimé, ce qui veut dire qu'
...
Rien de plus facile pour vérifier la présence d 'éléments enfants :hasChildNodes(). Il suffit d'
...
La méthode insertBefore() permet d 'insérer un élément avant un autre. Elle reçoit deux paramètres : le premier est 
l 'élément à insérer, tandis que le deuxième est l'
élément avant lequel l 'élément va être inséré. Exemple : <p id = "myP" > Un peu de texte < a > et un lien </a></p >
    //--------------------------------------------------------------------
    <
    script >
...
-- -- -- - NB
Le JavaScript met à disposition insertBefore(), mais pas insertAfter().C 'est dommage car, bien que l'
...
onClick correspond à la fonction que vous allez définir et qui sera appelée à chaque fois que l 'utilisateur cliquera sur votre lien

stopPropagation() est une fonction de l 'objet que votre fonction reçoit en paramètre. Son rôle est par contre très différent, car il 
nous permet d 'empêcher la propagation de l'
événement vers son parent

Les Serveur Web
Un service web est un programme s 'exécutant sur un serveur accessible depuis Internet et répondant à des demandes, 
appelées requêtes.
...
Les codes de 400 à 499 sont des codes d 'erreur liés à l'utilisation du service web
Les codes de 500 à 599 sont des codes d 'erreur venant du service web

200: indique que tout s 'est bien passé ;

201: indique que tout s 'est bien passé et qu'
une nouvelle ressource a bien été créée;

204: indique que tout s 'est bien passé mais qu'
aucun résultat n 'est renvoyé ;

400: indique qu 'une requête est erronée ; 

401: indique que l 'utilisateur n'
est pas authentifié, alors que c 'est nécessaire ;

403: indique que l 'utilisateur n'
a pas le droit d 'accéder à cette ressource ;
...
API signifie Application Programming Interface.Il s 'agit d'une interface mettant à disposition des points 
d 'accès vers les ressources de l'application

AJAX signifie en fait Asynchronous JavaScript And XML.Il s 'agit d'un ensemble d 'objets et de fonctions 
mis à disposition par le langage JavaScript, afin d 'exécuter des requêtes HTTP de manière asynchrone

UNSENT(code 0): l 'objet est prêt mais la méthode  open()  n'
a pas encore été appelée;
...
min / max: fonctionne avec des champs de type nombre ou dateCela permet de définir une valeur minimum et une
valeur maximum autorisées;
required: fonctionne avec à peu près tous les types de champs Cela rend obligatoire le remplissage de ce champ;
step: fonctionne avec les dates ou les nombres Cela permet de définir une valeur d 'incrément lorsque vous changez
la valeur du champ via les flèches
minlength / maxlength: fonctionne avec les champs textuels(text, url, tel, email...) Cela permet de
définir un nombre de caractères minimum et maximum autorisé. <
    input type = "text"
pattern = "[0-9]{,3}" / >
...
Lorsque l 'on récupère une  Promise , on peut utiliser sa fonction  then()  pour exécuter du code dès que la promesse est résolue, et sa fonction  catch()  pour exécuter du code dès qu'
une erreur est survenue.

Le linter est un programme qui va analyser notre code et détecter les erreurs de syntaxe, les variables non utilisées, les variables qui n 'existent pas, la mauvaise organisation du code, le non-respect des bonnes pratiques d'
écriture de code...

Creer un site web pour presenter votre cv
formation
competences
projet
technos